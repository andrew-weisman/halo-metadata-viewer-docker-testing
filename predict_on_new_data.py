...
def run_prediction(chosen_ensemble_model, X_test, prediction_method, encoder, df_to_analyze, independent_vars, dependent_var, test_idx_generalization, alpha=0.05, k=30, num_pca_components=0.95):
    ...
    if prediction_method == 'Original (RMSE)':
...
def main():
    ...
    uploaded_input_file = st.file_uploader("Upload a CSV file with new data on which to predict (optional):", type=['csv', 'xls', 'xlsx'])

    # Allow the user to load new data into the editable dataframe to be used for prediction.
    if st.button(':arrows_counterclockwise: Load uploaded input file', use_container_width=True, disabled=(uploaded_input_file is None)):
    ...
    st.session_state[df_editor_key].dataframe_editor(column_config=column_config)

    # Allow the user to select the prediction method.
    key = st_key_prefix + 'prediction_method'
    prediction_method_options = ['Original (RMSE)', 'Bias + std', 'Locally adaptive conformal', 'All methods']
    ...
    #### TODO: Add other settings to the actually_used_settings below...
    if st.button("Run prediction", use_container_width=True):
        ...
        if X_test.empty:
            ...
            st.session_state[st_key_prefix + 'actually_used_settings_for_run_prediction_button'] = {
                'chosen_ensemble_model': copy.deepcopy(st.session_state[st_key_prefix_select_model + 'actually_used_settings_for_select_model_for_analysis_button']),
                'edited_df': X_test.copy(),
            }
    ...
    if key not in st.session_state:
        if st_key_prefix_study_feature_importance + 'joined_df' in st.session_state:
            ...
            st.session_state[key] = feature_columns[0]
    x_axis_col_for_prediction_plot = st.selectbox("Select x-axis variable for plot:", options=feature_columns, key=key)

    # Define prediction methods and column mappings
    prediction_methods = ['Original (RMSE)', 'Bias + std', 'Locally adaptive conformal']
    method_to_col_map = {
        'Original (RMSE)': 'Prediction (rmse)',
        'Bias + std': 'Prediction (bias + std)',
        'Locally adaptive conformal': 'Prediction (conformal)',
    }
    method_to_low_col_map = {
        'Original (RMSE)': 'Lower bound (95%) (rmse)',
        'Bias + std': 'Lower bound (95%) (bias + std)',
        'Locally adaptive conformal': 'Lower bound (95%) (conformal)',
    }
    method_to_high_col_map = {
        'Original (RMSE)': 'Upper bound (95%) (rmse)',
